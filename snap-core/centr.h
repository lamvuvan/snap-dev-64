...
template <class PGraph> double GetFarnessCentr(const PGraph& Graph, const int64& NId, const bool& Normalized=true, const bool& IsDir=false);

template <class PGraph> double GetFarnessCentrMP(const PGraph& Graph, const int64& NId, const bool& Normalized=true, const bool& IsDir=false);

/// Returns weighted Farness centrality of a given node \c NId.
/// Farness centrality of a node is the average shortest path length to all other nodes that reside is the same connected component as the given node.
double GetWeightedFarnessCentr(const PNEANet Graph, const int64& NId, const TFlt64V& Attr, const bool& Normalized=true, const bool& IsDir=false);

/// Returns Closeness centrality of a given node NId.
/// Closeness centrality of a node is defined as 1/FarnessCentrality.
template <class PGraph> double GetClosenessCentr(const PGraph& Graph, const int64& NId, const bool& Normalized=true, const bool& IsDir=false);
template <class PGraph> double GetClosenessCentrMP(const PGraph& Graph, const int64& NId, const bool& Normalized=true, const bool& IsDir=false);
/// Returns Closeness centrality of a given node \c NId. 
/// Closeness centrality of a node is defined as 1/FarnessCentrality.
double GetWeightedClosenessCentr(const PNEANet Graph, const int64& NId, const TFlt64V& Attr, const bool& Normalized=true, const bool& IsDir=false);
...
template <class PGraph> int64 GetNodeEcc(const PGraph& Graph, const int64& NId, const bool& IsDir=false);
...
template<class PGraph> void GetBetweennessCentr(const PGraph& Graph, TIntFlt64H& NIdBtwH, const double& NodeFrac=1.0, const bool& IsDir=false);
/// Computes (approximate) weighted Node Beetweenness Centrality based on a sample of NodeFrac nodes.
/// @param NIdBtwH hash table mapping node ids to their corresponding betweenness centrality values.
/// @param NodeFrac quality of approximation. NodeFrac=1.0 gives exact betweenness values.
void GetWeightedBetweennessCentr(const PNEANet Graph, TIntFlt64H& NIdBtwH, const TFlt64V& Attr, const double& NodeFrac=1.0, const bool& IsDir=false);
...
/// @param NodeFrac quality of approximation. NodeFrac=1.0 gives exact betweenness values.
template<class PGraph> void GetBetweennessCentr(const PGraph& Graph, TIntPrFlt64H& EdgeBtwH, const double& NodeFrac=1.0, const bool& IsDir=false);
/// Computes (approximate) weighted Edge Beetweenness Centrality based on a sample of NodeFrac nodes.
/// @param EdgeBtwH hash table mapping edges (pairs of node ids) to their corresponding betweenness centrality values.
/// @param NodeFrac quality of approximation. NodeFrac=1.0 gives exact betweenness values.
void GetWeightedBetweennessCentr(const PNEANet Graph, TIntPrFlt64H& EdgeBtwH, const TFlt64V& Attr, const double& NodeFrac=1.0, const bool& IsDir=false);
/// Computes (approximate) Node and Edge Beetweenness Centrality based on a sample of NodeFrac nodes.
/// @param NIdBtwH hash table mapping node ids to their corresponding betweenness centrality values.
/// @param EdgeBtwH hash table mapping edges (pairs of node ids) to their corresponding betweenness centrality values.
/// @param NodeFrac quality of approximation. NodeFrac=1.0 gives exact betweenness values.
template<class PGraph> void GetBetweennessCentr(const PGraph& Graph, TIntFlt64H& NIdBtwH, TIntPrFlt64H& EdgeBtwH, const double& NodeFrac=1.0, const bool& IsDir=false);
/// Computes (approximate) weighted Node and Edge Beetweenness Centrality based on a sample of NodeFrac nodes.
/// @param NIdBtwH hash table mapping node ids to their corresponding betweenness centrality values.
/// @param EdgeBtwH hash table mapping edges (pairs of node ids) to their corresponding betweenness centrality values.
/// @param NodeFrac quality of approximation. NodeFrac=1.0 gives exact betweenness values.
void GetWeightedBetweennessCentr(const PNEANet Graph, TIntFlt64H& NIdBtwH, TIntPrFlt64H& EdgeBtwH, const TFlt64V& Attr, const double& NodeFrac=1.0, const bool& IsDir=false);
...
template<class PGraph> void GetBetweennessCentr(const PGraph& Graph, const TInt64V& BtwNIdV, TIntFlt64H& NodeBtwH, const bool& DoNodeCent, TIntPrFlt64H& EdgeBtwH, const bool& DoEdgeCent, const bool& IsDir);
...
void GetEigenVectorCentr(const PUNGraph& Graph, TIntFlt64H& NIdEigenH, const double& Eps=1e-4, const int64& MaxIter=100);

/// PageRank
/// For more info see: http://en.wikipedia.org/wiki/PageRank
template<class PGraph> void GetPageRank(const PGraph& Graph, TIntFlt64H& PRankH, const double& C=0.85, const double& Eps=1e-4, const int64& MaxIter=100);
template<class PGraph> void GetPageRank_v1(const PGraph& Graph, TIntFlt64H& PRankH, const double& C=0.85, const double& Eps=1e-4, const int64& MaxIter=100);
#ifdef USE_OPENMP
...
int64 GetWeightedPageRank(const PNEANet Graph, TIntFlt64H& PRankH, const TStr& Attr, const double& C=0.85, const double& Eps=1e-4, const int64& MaxIter=100);
#ifdef USE_OPENMP
...
template<class PGraph> void GetHits(const PGraph& Graph, TIntFlt64H& NIdHubH, TIntFlt64H& NIdAuthH, const int64& MaxIter=20);
#ifdef USE_OPENMP
...
template<class PGraph>
void GetPageRank_v1(const PGraph& Graph, TIntFlt64H& PRankH, const double& C, const double& Eps, const int64& MaxIter) {
...
template<class PGraph>
void GetPageRank(const PGraph& Graph, TIntFlt64H& PRankH, const double& C, const double& Eps, const int64& MaxIter) {
  const int64 NNodes = Graph->GetNodes();
...
template<class PGraph>
void GetBetweennessCentr(const PGraph& Graph, TIntFlt64H& NodeBtwH, TIntPrFlt64H& EdgeBtwH, const double& NodeFrac, const bool& IsDir) {
  TInt64V NIdV;  Graph->GetNIdV(NIdV);
  if (NodeFrac < 1.0) { // calculate beetweenness centrality for a subset of nodes
    NIdV.Shuffle(TInt::Rnd);
    for (int64 i = int64((1.0-NodeFrac)*NIdV.Len()); i > 0; i--) {
