...
void ParseArgs(int &argc, char *argv[], TStr &InFile, TStr &OutFile,
    ...
    InFile = Env.GetIfArgPrefixStr("-i:", argv[2],
                                   "Input graph path");
    OutFile = Env.GetIfArgPrefixStr("-o:", argv[4],
                                    "Output graph path");
...
void ReadGraph(TStr &InFile, bool &Directed, bool &Weighted, bool &Verbose, PWNet &InNet) {
    ...
    try {
        while (!FIn.Eof()) {
            ...
            int64 SrcNId = Tokens[0].GetInt64();
            int64 DstNId = Tokens[1].GetInt64();
            double Weight = 1.0;
...
void WriteOutput(TStr &OutFile, TInt64FltVH &EmbeddingsHV, TVVec <TInt, int64> &WalksVV,
                 bool &OutputWalks) {
    TFOut FOut(OutFile);
    if (OutputWalks) {
