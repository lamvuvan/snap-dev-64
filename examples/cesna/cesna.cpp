...
int main(int argc, char *argv[]) {
    ...
    const TStr ANameFNm = Env.GetIfArgPrefixStr("-n:", "./1912.nodefeatnames",
                                                "Input file name for node attribute names");
    ...
    const double AttrWeight = Env.GetIfArgPrefixFlt("-aw:", 0.5,
                                                    "We maximize (1 - aw) P(Network) + aw * P(Attributes)");
    const double LassoWeight = Env.GetIfArgPrefixFlt("-lw:", 1.0,
                                                     "Weight for l-1 regularization on learning the logistic model parameters");
    ...
    const double MinFeatFrac = Env.GetIfArgPrefixFlt("-mf:", 0.0,
                                                     "If the fraction of nodes with positive values for an attribute is smaller than this, we ignore that attribute");
